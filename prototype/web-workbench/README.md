...
- 钢琴卷帘已落地，但所有音符都是用户手工创建（source=manual）；等接入 Basic Pitch / Demucs 后端后才会有 source=transcription 的转录音符。已支持量化、1/8 拍、三连音、附点、Swing 网格、"量化"按钮、非破坏混音参数（trim/fade）与 A/B 试听切换（edited/original）。
- stem 混音器已落地（含 trim/fade），但 drums/bass/other 是占位 stem（无分离音频），等接入 Demucs / Basic Pitch 后端后才会实际播放与转录；当前 master stem 的 trim/fade 通过 Web Audio API `linearRampToValueAtTime` 真实生效，占位 stem 只保存参数。
