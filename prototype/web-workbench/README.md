...
- 撤销/重做栈（EditGraph 第一版）：新建/删除歌词、新建/删除休止、修正/恢复和弦、共享边拖动/微调、歌词块拖动/拉伸都会记录撤销点。Ctrl+Z 撤销，Ctrl+Shift+Z 或 Ctrl+Y 重做；按钮在顶部工具栏。
...
- 项目文件尚未包含锁定状态、读音、音素或演唱草稿。
- 缩放锚点（鼠标位置或播放头锚定）、自动滚动跟随播放头和字段级锁定尚未实现。
...
- 三平台浏览器和高 DPI/输入法测试尚未完成；本轮新交互（撤销/重做、歌词块拖动/拉伸）尚未经过真实浏览器回归。
