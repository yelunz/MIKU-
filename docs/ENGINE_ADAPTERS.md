...
| 4 | VOCALOID6 6.13.0 MIDI 降级 | VOCALOID6 Editor 6.13.0 完整版（Standalone） | `.mid` + `.vocaloid6-loss.json` sidecar | 无（Python 标准库，复用 MIDI 基线导出器） |
...
# 4. VOCALOID6 6.13.0 MIDI 降级导出（MIDI + sidecar 损失报告）
python tools/export_vocaloid6.py <project.json> <output.mid>
# 导出后在 <output.mid> 旁边生成 <output.mid>.vocaloid6-loss.json
```
...
### 3.5 VOCALOID6 6.13.0 MIDI 降级

VOCALOID6 Editor 6.13.0 完整版（Standalone）没有公开稳定的脚本 API，
原生格式 `.vpr` 是专有二进制，因此适配器只走 MIDI 降级路径：复用 MIDI
基线导出器的字节级生成逻辑，在 Track 0 / Track 1 各注入一条 track name
meta event（`FF 03`，分别为 `VOCALOID` / `Main Vocal`），并生成 sidecar
字段损失报告 JSON（`<output>.vocaloid6-loss.json`）。歌词编码为 UTF-8
（V6.2+ 支持，用户需在导入对话框手动选择 UTF-8）。完整字段损失报告、
编码建议、用户工作流与实机验证清单见 `docs/VOCALOID6_ADAPTER.md`。
...
| `RestEvent` | 隐式退化 | VOCALOID6 把休止表达为音符间隙，无显式休止事件 |
| `LyricRegion` 容器 | 完全丢失 | 只有音节级歌词写入 MIDI；容器分组与语言信息丢失 |
| `source_audio` | 完全丢失 | MIDI 不承载音频引用 |
| `tempo_map.first_beat_seconds` | 完全丢失 | VOCALOID6 假设第一个 tempo 在 position 0 |
| `stem_tracks` 非破坏混音参数 | 完全丢失 | VOCALOID6 不支持 stem mixer |

保留字段：`note.pitch`（MIDI tone）/ `note.velocity`（0-127）/ 音符时序
（anchor tick）/ `syllable.text`（`FF 05` UTF-8）/ `tempo_map.bpm`
（`FF 51`）/ 拍号（`FF 58`）。

## 4. 已知限制

- **VOCALOID6 6.13.0 适配**：已实现 MIDI 降级适配器
  （`tools/export_vocaloid6.py`）与 sidecar 损失报告，但**实机验证待
  完成**——用户尚未提供 VOCALOID6 6.13.0 完整版测试环境。V5/V4/V3 旧
  版不保证识别 MIDI 歌词；详见 `docs/ADAPTER_CAPABILITY_MATRIX.md`
  第 5 节与 `docs/VOCALOID6_ADAPTER.md`。
...
- **VOCALOID6 复用 MIDI 基线**：VOCALOID6 适配器（`tools/export_vocaloid6.py`）
  不重新实现 MIDI 字节生成，而是直接 import `tools/export_midi.py` 的
  `build_tempo_track` / `build_main_track` / `build_smf` 等函数，只在 Track 0 /
  Track 1 头部各注入一条 track name meta event（`FF 03`），并在导出时生成
  sidecar 损失报告 JSON。不读写 `.vpr` / `.vsqx`，不依赖脚本 API。
